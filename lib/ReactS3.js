...
class S3FileUpload {
    static async uploadFile(file, fName, config) {

        const file_ext = file.name.substr(file.name.lastIndexOf('.')+1,file.name.length);
        const file_name = fName + '.' + file_ext;
        
        // Error Thrower :x:
        throwError(config, file);

        const fd = new FormData();
        const key = `${config.dirName ? config.dirName + "/" : ""}${file_name}`;
        ...
        return Promise.resolve({
            ...
            key: `${config.dirName ? config.dirName + "/" : ""}${file_name}`,
